  ...
  onMessage(e) {
    ...
    switch (command.command) {
      ...
      case "HTML":
        ...
        this.props.onChange && this.props.onChange(command.html);
        break;
      case "onFocus":
  ...
  renderButton() {
    ...
    if (this.commands && this.commands.length > 0) {
      return this.commands.map((item, index) => {
        let isChecked =
          this.state.editorStates.indexOf(item.name) != -1 ? true : false;
        ...
        return (
          ...
          >
            <Text style={[fontColor, styles.text]}>
              {item.name}
              {/* {item.glyph} */}
            </Text>
          </TouchableOpacity>
